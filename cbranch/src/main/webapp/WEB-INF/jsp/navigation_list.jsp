...
<div id="sidebar"><a href="#" class="visible-phone"><i class="icon icon-home"></i> Dashboard</a>
  ...
  <li class="submenu"> <a href="#"><i class="icon icon-th-list"></i> <span>项目案例</span> <span class="label label-important">2</span></a>
       <ul>
       <li><a href="<%=basePath%>project/addProject">添加项目案例</a></li>
    ...
    <li class="submenu"><a href="#"><i class="icon icon-tint"></i> <span>广告管理</span><span class="label label-important">2</span></a>
      <ul>
        <li><a href="<%=basePath%>business/save">添加广告</a></li>
        <li><a href="<%=basePath%>business/list">广告列表</a></li>
      </ul>
    </li>
  ...
  <div id="content-header">
    ...
    <div id="breadcrumb">
    	<a href="<%=basePath%>user/index" class="tip-bottom" data-original-title="前往首页">
    		<i class="icon-home"></i> 首页</a> <a href="<%=basePath %>navigation/toNavigati">导航栏列表</a> 
    		<a href="<%=basePath %>navigation/toNavigati" class="current">导航栏列表</a>
    </div>
     <!--树形指示栏 end-->
    ...
    <div class="row-fluid">
      <div class="span11">      	
          ...
          <div class="widget-title">            
        		...
        		<a href="<%=basePath%>navigation/addNavigation" class="btn btn-success">添加导航</a>        		
          </div>
          <form>
          <div class="widget-content nopadding">
            <table class="table table-bordered data-table table-list">          	
              <thead>
               ...
               <tbody>
                 <c:forEach items="${navigation}" var="nav" varStatus="staturs">
                	<tr>
                	 <td><input type="checkbox" name="chbox" value="${nav.id}"/> </td>
				     <td><c:out value="${nav.name }"></c:out></td>
				     <td><c:out value="${nav.title }"></c:out></td>
				     <td><a href="http://<c:out value="${nav.url}"></c:out>" target="_blank"><c:out value="${nav.url}"></c:out></a></td>				                 
				     <td class="center"><c:out value="${nav.power}"></c:out> </td>
				     <td><a href="<%=basePath%>navigation/doDeleteNavigation?id=${nav.id}"class="btn btn-danger btn-remove" id="btn-del">删除</a>&nbsp;&nbsp;
				         <a href="<%=basePath %>navigation/updateNavigation?id=${nav.id}&title=${nav.title}&name=${nav.name}&url=${nav.url}&power=${nav.power}"
