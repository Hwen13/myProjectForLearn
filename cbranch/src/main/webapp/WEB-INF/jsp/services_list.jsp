...
<!--左边菜单栏 end-->
  <div id="content">
    ...
    <div id="breadcrumb"> <a href="<%=basePath%>user/index" class="tip-bottom" data-original-title="前往首页">
    	<i class="icon-home"></i> 首页</a> <a href="#">服务管理</a> 
    	<a class="current">服务列表</a> 
    </div>
    	<!--树形指示 end-->
			  		 ...
			  		 <c:otherwise>
				  	   <c:forEach items="${list}" var="ser">
					  		<tr>
					  			...
					  			<td>${ser.power}</td>
					  			<td><a class="btn btn-danger btn-remove" href="delete?id=${ser.id}">删除</a> &nbsp;&nbsp;<a href="change?id=${ser.id}" class="btn btn-success">修改</a></td>
					  		</tr>
				  	   </c:forEach>
			  		 </c:otherwise>
			  	  </c:choose>
