        ...
        <div class="span12">
          <div class="widget-box"> 
            <div class="widget-title">              
           	 <h5>业务列表 </h5>           
        			<a id="listDelete" class="btn btn-danger" style="margin-top: 3px;">批量删除</a>	
              ...
              <c:choose>
			  		 ...
			  		 <c:otherwise>
				  	   <c:forEach items="${list}" var="bus">
					  		<tr>
					  			<td>
					  			<input type="checkbox" name="chbox" value="${bus.id}"></td>
					  			<td>${bus.title}</td>
					  			<td><a><img src="<%=basePath%>${bus.remark}"/></a></td> 			
					  			<td>${bus.description}</td>
					  			<td>${bus.power}</td>
					  			<td><a href="delete?id=${bus.id}" class="btn btn-danger btn-remove">删除</a> &nbsp;&nbsp;<a href="change?id=${bus.id}" class="btn btn-success">修改</a></td>
					  		</tr>
				  	   </c:forEach>
			  		 </c:otherwise>
			  	  </c:choose>
